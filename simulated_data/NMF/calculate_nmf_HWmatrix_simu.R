...
option_list = list(
  ...
  make_option(c("-c", "--ct"), action="store", default="BLCA", type='character',
              help="select folder"),
  make_option(c("-f", "--minimum_factors"), action="store", default = 5,
              help="Minimum number of factors to optimiza the NMF"),
  make_option(c("-F", "--maximum_factors"), action="store", default = 25,
              help="Maximum number of factors to optimiza the NMF"),
  make_option(c("-p", "--no_cores"), action="store", default = 5,
...
factor_range = rev(seq(from = minimum_factor_range, to = maximum_factor_range, by = 5))

# Load data:
path = paste0("results/input_coefs_simu_withct/", ct, "/", file_name)
print(path)
nIter = as.numeric(sub(".*coef_([0-9]*)\\.csv", "\\1", file_name))
print(nIter)

resamp = read_csv(path)
dim(resamp)
resamp[1:5,1:5]
...
# save
folder = paste0("/slgpfs/projects/irb57/msalvadores/RMDSigs/results/sig_matrix_simu_withct/",ct, "/matH")
...
# save
folder = paste0("/slgpfs/projects/irb57/msalvadores/RMDSigs/results/sig_matrix_simu_withct/",ct, "/matW")
if (!file.exists(folder)){
  dir.create(file.path(folder), recursive = TRUE)
} 
