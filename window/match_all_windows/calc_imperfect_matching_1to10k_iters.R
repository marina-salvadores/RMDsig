...
all_counts = read_csv("results/windows/tri_composition/trint_composition_3062original_windows.csv")
dim(all_counts)
head(all_counts)
...
stoppingCriterion = 0.0000001    
maxIter = 100000  # to prevent endless loops (in reality this can be a very high #, this works quite fast)
iter=0
...
for(i in 1:10){
  ...
  write.csv(new, paste0("results/windows/tri_composition/tri_composition_original_windows_imperfect_matching_",as.character(maxIter),".csv"), row.names = F)
